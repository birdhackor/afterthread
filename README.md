...
| `/tools` | 工具 | 已安裝工具的清單（啟用/停用/刪除；每個工具可展開讀取／重新產生 AI 總結、提意見送出 AI 修訂）+「安裝新工具」的 KB 網頁安裝器（見下一節）。 |
| `/llm-logs` | AI 日誌 | 每一次 AI 互動（快速捕捉／AI 補齊／AI 進度更新／工具建置——安裝與 AI 修訂共用同一種建置 session／工具總結——安裝或修訂完成後產生總結、以及「重新產生總結」）的提示與回應紀錄，可展開查看每次嘗試，供除錯用（每則內容受 `LLM_LOG_BODY_MAX_CHARS` 截斷、超量的較早訊息會被省略、工具呼叫參數為約 200 字元預覽）。 |
