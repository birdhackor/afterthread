...
4. **結果**：「安裝完成」或「安裝失敗」，附 AI 自己的文字摘要，以及一個
   「查看 AI 日誌」連結——直接跳到 AI 日誌頁並展開這次建置工作的完整互動
   紀錄（提示、AI 回覆、每一次寫檔/讀檔/`run_shell` 呼叫的內容）。**失敗時
   這個連結是最主要的除錯入口**：AI 判定「尚未完成」的原因、`run_shell`
   實際測試時的錯誤輸出，都完整躺在裡面。
...
- **因此**：只安裝你信任的 OpenAPI 文件來源與指示內容，就像你只會執行信任
  ...
  能力/風險模型），不是尚未做完的安全機制。
- **貼進去的金鑰會被記錄**：安裝指示文字、以及 AI 每一次寫檔/`run_shell`
  呼叫的完整參數，都會被記錄進這次建置工作的 AI 日誌（提示/回應的完整內容，
  用於除錯）。也就是說，你貼給 AI 的第三方 API key 不只會進到工具自己的
  `.env`，也會原樣出現在 AI 日誌裡——日誌預設只存在後端記憶體、隨程序重啟
  清空，但如果設定了 `LLM_LOG_FILE`，就會連同其他 AI 互動一起落地到那個
  檔案。不要因為「金鑰只會寫進工具的 .env」而誤以為它不會出現在別處。
