...
- `GET /api/tools/jobs/{job_id}` — 輪詢一個工具工作（安裝**或**修訂）的狀態
  ...
  記憶體）則回 404。安裝與修訂共用同一張工作表、同一個輪詢端點，回應形狀完全
...
- **依意見修訂既有工具**（`run_revise`，設計依據見 `docs/web-v4-decisions.md`
  ...
  目錄裡的那一包。三件事值得知道：
  - 複製時**排除 `.env` 與 AI 總結 sidecar**（兩者在任何層級都排除）。`.env` 一定
    要排除：它的值就在「已知秘密」集合裡，複製進去必然被安裝驗證的「檔案不得內嵌
    秘密值」擋下，等於這個工具再也修訂不了。原本的 `.env` 以**文字**保留（內容與
    行序原樣，換行正規化與非法位元組替換沿用共用的受限讀取器；工具實際載入到的值
    完全不變，因為 runtime 走的是同一條解碼路徑），在驗證通過之後、換裝之前寫回去
    （模型自己寫的 `.env` 一律被覆蓋；系統提示已明說「`.env` 由後端保管、不要重寫、
    也不要依賴改寫它」）。要改秘密值請直接手動編輯該工具的 `.env`，但**手寫的值
    長度至少要 6 字元**（遮蔽器的下限，比照安裝表單），否則修訂會在開工前就以
    「有值過短、無法遮蔽」拒絕——遮不掉的值會隨 `run_shell` 的輸出進到提示與 AI
    日誌，不做就只剩下外洩一途。修訂期間，`.env` 的每個值都會被登記為「進行中的
    秘密」，因為換裝那一瞬間舊套件被改名成隱藏備份目錄，而已知秘密掃描**跳過隱藏
    目錄**，不補這一手就會有一段沒被遮蔽的空窗。
