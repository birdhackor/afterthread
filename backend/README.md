...
uv run uvicorn app.main:app --port 8000        # 啟動 API（DB 不存在會自動建立）
uv run uvicorn app.main:app --port 8000 --reload   # 開發時加 --reload 自動重載
...
所有數值型設定都在 `app/config.py` 用 pydantic-settings 的 `Field(..., ge=/le=/gt=)`
...
所有路由掛在 `/api` 前綴下（見 `app/main.py`）。完整 request/response schema
...
**Items CRUD**（`app/routers/items.py`，前綴 `/api/items`）
...
**AI workflows**（`app/routers/ai.py`）
...
- **SQLite-only guard + 存活探測（persistence probe）**：`app/db.py` 的
...
- **FK pragma**：SQLite 預設不強制 foreign key 約束，且是 per-connection 設定。
  `app/db.py` 在每個連線的 `connect` event 上送 `PRAGMA foreign_keys=ON`，避免例如
...
- **supersede-not-delete**：`decisions`／`rationale`／`alternatives`／
  ...
  寫。`app/services/memory_ai.merge_with_supersede` 逐行比對：舊內容的每一行只要
  完整出現在新內容裡就視為保留；否則把舊內容整段接到新內容之後、掛上帶日期的
  `superseded` 標記，確保之前的決策脈絡不會無聲消失（合併結果另外有存量上限，超出
  時從最舊的一段開始截斷，並留下明顯的截斷標記）。
- **schema-guided LLM 輸出 + 修正重試**：`app/services/llm.generate_structured`
