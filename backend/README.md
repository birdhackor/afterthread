...
- **依意見修訂既有工具**（`run_revise`，設計依據見 `docs/web-v4-decisions.md`
  ...
  - 複製時**排除套件根層的 `.env`**（AI 總結 sidecar 則在任何層級都排除）。根層
    ...
    **長度至少要 6 字元**（遮蔽器的下限，比照安裝表單；補救是加長或移出 `.env`），
    而且**值在「決定它的那一行賦值」裡的寫法要逐字等於它本身**——像 `KEY="ab'cd\"ef"`
    這種跳脫會真的生效的寫法，解析出來的值在那一行的右邊根本找不到（**看的是那一行、
    不是整份檔案**：別處的一行註解剛好寫著同一串字，不會讓那行賦值變得遮得掉；
    dotenv 後出現者勝，所以看的是最後一行指派該 key 的賦值），找不到可對應的賦值行
    （跨行引號值、續行）同樣拒絕，補救是簡化該值的引號與跳脫寫法（安裝路徑本來就拒絕
    寫出這種拼法，這是同一條規則套在繼承端；反過來說，這套系統自己寫出來的 `.env`
    一定過得了這一關）。理由都一樣：
