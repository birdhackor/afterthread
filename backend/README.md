...
- **依意見修訂既有工具**（`run_revise`，設計依據見 `docs/web-v4-decisions.md`
  ...
  - 複製時**排除套件根層的 `.env`**（AI 總結 sidecar 則在任何層級都排除）。根層
    ...
    `.ENV` 之類大小寫變體，只有在它與 `.env` 是同一個檔案（同 inode，case-insensitive
    檔案系統上正是如此）時才排除**；在 case-sensitive 檔案系統上它是不同的普通檔案，
    照常複製、照常出貨。要改秘密值請直接手動編輯該工具的 `.env`，但那些手寫的值必須
    是遮蔽器**遮得掉**的，否則修訂在開工前就被拒絕，兩種情形各有自己的訊息與補救：
    **長度至少要 6 字元**（遮蔽器的下限，比照安裝表單；補救是加長或移出 `.env`），
    而且**值在 `.env` 裡的寫法要逐字等於它本身**——像 `KEY="ab'cd\"ef"` 這種跳脫會
    真的生效的寫法，解析出來的值在檔案原文裡根本找不到，補救是簡化該值的引號與跳脫
    寫法（安裝路徑本來就拒絕寫出這種拼法，這是同一條規則套在繼承端）。理由都一樣：
