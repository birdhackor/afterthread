...
`context-memory` 目前不透過 PyPI 發佈(這是一個私人 repo),所以要先從原始碼建置一次 wheel 檔,再用它啟動服務。
...
完成後,`backend/dist/` 目錄下會出現一個內嵌了前端 production build 的 `context_memory-*.whl`(以及對應的原始碼套件 sdist)。
...
不想每次都打一長串 `uvx --from ...`,可以裝成一個固定指令,之後直接打 `context-memory`:
...
`context-memory --help` 可以看完整說明。

| 旗標 | 預設值 | 對應環境變數 |
| --- | --- | --- |
| `--host` | `127.0.0.1`(只綁本機,不對外網開放——這是單人本機工具,不是要曝露在網路上的服務) | `CONTEXT_MEMORY_HOST` |
...
- **資料目錄**:預設是 `$XDG_DATA_HOME/context-memory`,沒設定 `XDG_DATA_HOME` 時退回 `~/.local/share/context-memory`。可以用 `--data-dir <path>` 或環境變數 `CONTEXT_MEMORY_DATA_DIR` 覆寫。第一次啟動時,若目錄不存在會自動建立(權限設為只有你自己能讀寫);已存在的目錄則不會被動權限。SQLite 資料庫檔與工具目錄(見 [KB 工具安裝指南](kb-tools.md))都會落在這裡。
