...
- **`.state.json` 壞掉不會被當成「沒設定」**。檔案在但讀不出來（截斷、不是 JSON、
  ...
  - **還是一般檔案，只是內容壞了**（截斷、不是 JSON、`enabled` 不是布林、超過上限）
    ——按一次開關就好。PATCH 不讀這個檔案，直接覆蓋成乾淨的一份。
  - **已經不是一般檔案了**（被換成 symlink／FIFO／目錄）——**按開關沒有用**。發佈器
    在寫入前會拒絕任何既存的非一般檔（那是刻意的寫入邊界：絕不透過 symlink 或 FIFO
    寫出去），所以 PATCH 只會失敗回 404，那個東西原封不動。這時只能自己把它移除，
    移除後就回到上面那條舊安裝的 fallback 規則。
