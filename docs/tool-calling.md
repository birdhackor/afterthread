...
3. **再確認「這個工具現在還是開著的嗎」**：啟用狀態存在套件自己的
   `.state.json`（不在 `tool.json` 裡，見下方說明），執行前會再問一次——問的是
   **跟清單、廣告同一條優先序**：狀態檔在就以它為準，不在就退回 `tool.json` 的舊
...
「啟用／停用」是套件的**可變狀態**，存在套件自己的 `.state.json`（隱藏檔，內容
就是 `{"enabled": true}` 或 `{"enabled": false}`），**不在 `tool.json` 裡**。
...
- **舊安裝不用做任何事**。沒有 `.state.json` 的套件，讀的還是 `tool.json` 裡那個
  舊的 `enabled` 欄位（預設 true）。這就是全部的遷移——沒有啟動時的掃描改寫，讀取
  永遠不會寫入。第一次按開關才會產生 `.state.json`，而且**不會**順手去刪 `tool.json`
  裡那個已經沒人讀的舊欄位：為了整潔去改寫 manifest，正好會移動這次改動要固定住的
  那個身分。
- **`.state.json` 壞掉不會被當成「沒設定」**。檔案在但讀不出來（截斷、不是 JSON、
  `enabled` 不是布林、被換成 symlink／FIFO／目錄），代表**你的意圖不明**，這時
  後端把那個工具列成無效、並在列表的錯誤欄顯示 `.state.json exists but is not
  readable`，同時當成停用。刻意不退回「預設開啟」：那等於把一個有人特意關掉的工具
  重新交到模型手上。修法**看它壞成什麼樣子**，兩類不一樣：
  - **還是一般檔案，只是內容壞了**（截斷、不是 JSON、`enabled` 不是布林、超過上限）
    ——一次 `PATCH /api/tools/{name}` 就修好：它不讀這個檔案，直接覆蓋成乾淨的一份。
    **但畫面上那顆開關按不下去**：這個套件此刻正是「無效」的，而 UI 對每一個無效列
    都停用開關。所以實務上兩條路——**自己把 `.state.json` 刪掉**（回到上面那條舊安裝
...
- **整個套件目錄被換成 symlink ⇒ 那個工具一律不跑**，而且清單上會顯示成「無效 ＋
  關著」。後端在接上任何檔名之前就拒絕看這種路徑（否則 `<連結>/.state.json` 會跟著
...
- **啟用狀態同樣不進工作區，但會被帶過去**。`.state.json`（見下一小節）跟 sidecar
