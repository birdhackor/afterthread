...
- **`.env` 不進工作區，也不由模型改寫**。複製套件到暫存目錄時，`.env` 與 AI 總結
  sidecar 在任何層級都會被排除。`.env` 一定得排除，理由很實際：它的值就在「已知
  秘密」集合裡，複製進去之後必然會被安裝驗證的「檔案不得內嵌秘密值」那一關擋下
  ——等於這個工具從此再也修訂不了。後端把 `.env` 的**原文以文字留在記憶體裡**
  （內容與行序原樣；換行正規化成 LF、非法位元組換成 U+FFFD，這是共用受限讀取器
  的既有行為。刻意不改成 raw bytes：runtime 載入 `.env`、遮蔽器掃 `.env`、這裡的
  保留，走的都是同一條解碼，所以工具實際拿到的值本來就一模一樣，改成保留位元組
  只會多一條讀寫路徑而看不到任何差別），等新套件通過驗證之後、換裝之前寫回去
  （模型自己寫的 `.env` 一律被覆蓋）。系統
