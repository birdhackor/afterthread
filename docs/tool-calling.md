...
- **根層 `.env` 不進工作區，也不由模型改寫**。複製套件到暫存目錄時，**套件根層的
  ...
  - **值在 `.env` 裡的寫法要逐字等於它本身**：`KEY="ab'cd\"ef"` 這種跳脫真的會生效的
    寫法，解析出來是 `ab'cd"ef`，而檔案原文裡沒有這串字——遮蔽器只認得解析後的值，
    所以 builder 一句 `cat .env`（正式套件在暫存區外，但 `run_shell` 構得到）就把
    遮不掉的原文帶回對話。補救是簡化該值的引號與跳脫寫法。安裝路徑本來就拒絕**寫出**
    這種拼法（`_dotenv_serialize_value` 會直接放棄），這是同一條規則套在繼承端。
...
- **巢狀的 `.env` 是普通套件內容，會照常複製**。工具自己讀的 `config/.env` 之類的檔案
  ...
  只有在它與 `.env` 是**同一個檔案**（同 inode——case-insensitive 檔案系統上兩個名字
  就是一個檔案）時才排除；在 case-sensitive 檔案系統上它是另一個普通檔案，照常複製、
  照常出貨（後端只會還原正好叫 `.env` 的那一個，所以誤排除等於把它靜默刪掉）。
