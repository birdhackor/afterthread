...
- **安裝網址只留出處**：記在 sidecar 裡、也餵給模型的那個 OpenAPI 網址，只保留
  `scheme://主機/路徑`；`?` 後面的查詢字串、`#` 後面的片段、以及 `user:pass@` 這種
  帳密段一律**整段丟掉**，並在尾巴補一個標記讓你知道有東西被拿掉了（無法解析的網址
  就整個留空，絕不原樣寫出去）。這不是「遮蔽」而是「不留」，因為遮蔽在這裡本來就
  救不了：presigned 連結或你臨時貼上的 token 從來沒被登記成已知秘密，遮蔽器不知道
  那是什麼；就算登記過，URL 裡常常是編碼過的樣子（登記的是 `abc+/`、網址寫的是
  `abc%2B%2F`），比對不會中。反正之後沒有任何流程會再去抓這個網址——它純粹是「這個
  工具是照哪份文件做的」的出處顯示——丟掉查詢字串不影響任何功能。已經存在的舊
  sidecar 不用處理：讀得出來照樣讀，下次按「重新產生」就會順手改寫成收斂後的樣子。
...
- **重新產生是同步的**：它只是一次單輪生成（沒有工具迴圈、沒有 shell），所以就
  ...
  像安裝那樣包成背景工作。
- 依使用者意見**修訂既有工具**（讓 AI 照你的回饋改寫這個套件，再迭代到定版）是
  下一個階段的功能，目前還沒有。
