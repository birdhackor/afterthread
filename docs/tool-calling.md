...
2. **先確認「這還是當初廣告出去的那個套件嗎」**：工具規格是在一次 AI 工作
   *開始*時就快照下來的，但真正執行是在模型呼叫它的那一刻（可能是幾分鐘之後），
   ...
   讀的檔案刪掉。順帶一提，「啟用／停用」開關會就地改寫 `tool.json`，所以在對話
   進行中切換開關也會讓那個工具在**這次對話剩下的呼叫**被拒——這是刻意選的保守
   方向（重跑一次即可），因為沒有任何比較寬鬆的判準能分辨「只是改了 enabled」與
   「整包被換掉」。
3. 啟動一個全新的**子行程**（subprocess）執行這個工具的 `entry`（例如
   `["python3", "run.py"]`），工作目錄設在這個工具自己的資料夾。
4. 參數 JSON 從子行程的標準輸入（stdin）餵進去，子行程印到標準輸出（stdout）
   的內容就是結果。
5. 子行程的環境變數是**從零打造**的（細節見第 3 節）。
6. 有逾時（`LLM_TOOL_TIMEOUT_SECONDS`，預設 60 秒）跟輸出長度上限
   （`LLM_TOOL_OUTPUT_MAX_CHARS`，預設 50000 字元，超過會截斷並標記
   `…[工具輸出過長已截斷]`）。
7. 不管工具本身是成功、失敗還是逾時，後端一律把結果包成一段文字（絕不讓工具
