...
- **根層 `.env` 不進工作區，也不由模型改寫**。複製套件到暫存目錄時，**套件根層的
  ...
  - **值在「決定它的那一行賦值」裡的寫法要逐字等於它本身**：`KEY="ab'cd\"ef"` 這種
    跳脫真的會生效的寫法，解析出來是 `ab'cd"ef`，而那一行的右邊沒有這串字——遮蔽器
    只認得解析後的值，所以 builder 一句 `cat .env`（正式套件在暫存區外，但 `run_shell`
    構得到）就把遮不掉的原文帶回對話。**看的是那一行，不是整份檔案**：檔案裡別處
    （例如一行註解）剛好也寫著同一串字，並不會讓那行賦值變得遮得掉。python-dotenv
    是**後出現者勝**，所以要看的是**最後**一行指派該 key 的賦值；同一個 key 前面那些
    被蓋掉的行不算數（它們解析不出任何已登記的值）。**找不到可對應的賦值行也一樣拒絕**
    ——跨行的引號值、續行、任何以「一行」讀不出來的寫法：讀得出來的才驗，讀不出來的就
    拒絕。補救都是簡化該值的引號與跳脫寫法。安裝路徑本來就拒絕**寫出**這種拼法
    （`_dotenv_serialize_value` 會直接放棄），這是同一條規則套在繼承端；反過來說，
    **凡是這套系統自己寫出來的 `.env` 一定過得了這一關**（它會把先前同名的行全部丟掉、
    自己的那行附在最後，而它寫得出來的三種拼法都逐字含有原值）。
