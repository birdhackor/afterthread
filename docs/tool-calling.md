...
2. **先確認「這還是當初廣告出去的那個套件嗎」**：工具規格是在一次 AI 工作
   *開始*時就快照下來的，但真正執行是在模型呼叫它的那一刻（可能是幾分鐘之後），
   而一般的 AI 工作**不在**工具工作的單一名額之內——所以一次 AI 修訂完全可能在
   這段期間把套件整包換掉。後端因此在廣告當下記下該套件 `tool.json` 的檔案身分
   （`dev`／`ino`／`ctime`，與修訂換裝前那道檢查同一個定義），執行前再取一次，
   對不上就**不執行**，直接回一段「這個工具在對話期間被換掉了，這次呼叫沒有跑」
   的結果字串（只講類別，不含名稱或路徑）。少了這道，模型會拿著舊的參數 schema
   去呼叫一個已經換成別的實作的 entry，而換裝的備份清理還可能把剛啟動的行程正在
   讀的檔案刪掉。這道檢查會被觸發的另一種情況是**你自己手動編輯 `tool.json`**
   （這是支援的行為）：就地改寫會推進檔案身分，所以那個工具在**這次對話剩下的
   呼叫**都會被拒——刻意選的保守方向（重跑一次即可），因為沒有任何比較寬鬆的
   判準能分辨「只是改了一個欄位」與「整包被換掉」。
3. **再確認「這個工具現在還是開著的嗎」**：啟用狀態存在套件自己的
   `.afterthread-state.json`（不在 `tool.json` 裡，見下方說明），執行前會再問一次——問的是
   **跟清單、廣告同一條優先序**：後端自己那份狀態檔在就以它為準，不在（或那個名字上
   放的是別人的檔案）就退回 `tool.json` 的舊
   `enabled` 欄位。「不在」不等於「沒意見」：一個舊安裝在對話中途被刪掉狀態檔
   （那正是下面寫的修法之一）時，讀回來的是 manifest 說的那個值。你在對話
   進行中把工具關掉，接下來的呼叫就會收到**另一段**結果字串——「這個工具在被端
   出去之後被停用了」。跟上面那段刻意分開：套件一個位元組都沒變，講成「被換掉」
   會把模型導去重讀一份根本沒動過的規格。
   （第 2、3 兩道檢查各問**兩次**：一次在呼叫剛進來時，一次緊貼在啟動子行程之前
   ——因為中間還夾著讀 `.env`、序列化參數與排隊等執行緒。第二次問的順序是反過來
   的：**身分檢查排在最後**，因為子行程的工作目錄是核心在啟動當下才從路徑解析的，
   落在那個縫隙裡的一次換裝會讓**新套件的程式碼**用舊契約跑起來，比晚一步看到開關
   嚴重得多。）
4. 啟動一個全新的**子行程**（subprocess）執行這個工具的 `entry`（例如
   `["python3", "run.py"]`），工作目錄設在這個工具自己的資料夾。
5. 參數 JSON 從子行程的標準輸入（stdin）餵進去，子行程印到標準輸出（stdout）
   的內容就是結果。
6. 子行程的環境變數是**從零打造**的（細節見第 3 節）。
7. 有逾時（`LLM_TOOL_TIMEOUT_SECONDS`，預設 60 秒）跟輸出長度上限
   （`LLM_TOOL_OUTPUT_MAX_CHARS`，預設 50000 字元，超過會截斷並標記
   `…[工具輸出過長已截斷]`）。
8. 不管工具本身是成功、失敗還是逾時，後端一律把結果包成一段文字（絕不讓工具
...
- **輸出遮蔽（已知秘密進對話前被遮）**：工具的輸出、或安裝過程中元工具的
  ...
  （畫面上會看到類似 `•••[秘密已遮蔽]•••` 的樣子）。三個界線要誠實講清楚：
  (1) 只遮**長度 6 字元以上**的值——太短的值不遮（會把一般文字也切碎），而
  安裝表單的秘密欄位同樣要求至少 6 字元，所以**極短的秘密目前無法被安全遮蔽、
  也不建議用**；(2) 比對的**只有值本身那個字串**，所以同一個秘密若在別處以另一種
  拼法出現（例如 `.env` 裡跳脫真的會生效的寫法），那份文字是遮不到的——修訂路徑
  因此在開工前就拒絕這種 `.env`（見 4.）；(3) **送進對話給模型看**的那條路徑是
  fail-closed（遮蔽若出錯就讓該步驟失敗，不會把原值放出去）；但**存進 AI 日誌**
  的那條是刻意 fail-open（紀錄器是純觀測者，遮蔽萬一出錯時寧可存下未遮蔽的原文，
  也不讓「記錄」這件事反過來弄壞真正的 LLM 呼叫）——所以日誌的遮蔽是常態保證、
  不是絕對保證。
...
目標 API 需要金鑰時，**該走的是安裝表單的「秘密欄位」**（一個名字 + 一個值），
而不是把值寫進指示文字——指示是原樣送進提示、也原樣落進 AI 日誌的，而那個值在
被貼上的當下還不是「已知秘密」，沒有任何一層遮蔽攔得到它。填了秘密欄位時，這個
值的處理原則是「盡量不讓模型直接拿到值」：
...
- **存在哪裡**：套件自己的 `.ai_meta.json`（隱藏檔）。工具本體就是一個檔案包，
  ...
  檔案不會被當成一個壞掉的工具列出來，也不會被送進任何提示（總結用的檔案清單
  跳過所有隱藏檔，所以總結不會把自己上一次的輸出當成原始碼再讀一遍）。
- **這個檔案是後端照固定格式重寫的，不是把記憶體裡的東西倒出來**：只有
  `summary`／`status`／`updated_at`／`llm_log_id`／`llm_log_process`／`origin`
  六個欄位，key 全是程式裡寫死的字面值。（`llm_log_process` 記的是「寫下那個
  `llm_log_id` 的是哪一個行程」：AI 日誌的 id 每個行程都從 0 重新數、環狀緩衝
  重啟即空，而這個檔案會把整數永久留著，所以重啟後同一個 id 指到的是別次互動。
  讀取時 token 對不上就不給連結——側檔本身不動，只是那條深連結不再提供。）你可以手動編輯它（這份文件與 README 都說可以），但下一次
...
- **重新產生是同步的**：它只是一次單輪生成（沒有工具迴圈、沒有 shell），所以就
  ...
  像安裝那樣包成背景工作。
### 啟用開關存在哪裡（`.afterthread-state.json`）

「啟用／停用」是套件的**可變狀態**，存在套件自己的 `.afterthread-state.json`（隱藏檔，
內容就是 `{"afterthread": "tool-state", "enabled": true}` 或同樣帶標記的 `false`），
**不在 `tool.json` 裡**。

那個 `"afterthread": "tool-state"` 是**所有權標記**，而且是有作用的：套件目錄是
**工具的**，工具本來就可能在裡面放自己的游標、快取或設定檔。後端只承認帶著這個
標記的檔案是自己的，其他一律當成「這裡沒有狀態檔」——不讀它的值、不覆蓋它、也不
刪掉它（見下面「那個名字上是我自己的檔案」）。

這件事看起來只是搬個位置，實際上是這個子系統一整類 bug 的根因修正。`tool.json`
的檔案身分（`dev`／`ino`／`ctime`）就是後端用來回答「這個路徑上還是我剛才看的那
一包嗎」的判準——修訂換裝前要問、寫總結側檔前要問、對話中途執行前要問。而
`enabled` 住在同一個檔案裡的時候，**按一次開關就等於就地改寫那個檔案**，判準跟著
位移，於是一次開關對每一道守衛來說都長得跟「整包被換掉」一模一樣：正在跑的子行程
從執行登記表上消失、進行中的修訂被判定「原工具被改動」而整場作廢、一趟總結 LLM
往返回來時側檔寫入被拒。搬出去之後，一次開關讓 `tool.json` **一個位元組都不變、
檔案身分也不動**，上面那些情境就不成立了，而不是各自被補一個特例。

幾個你會實際碰到的行為：

- **舊安裝多半不用做任何事**。沒有 `.afterthread-state.json` 的套件，讀的還是 `tool.json` 裡那個
  舊的 `enabled` 欄位（預設 true）。這就是全部的遷移——沒有啟動時的掃描改寫，讀取
  永遠不會寫入。第一次按開關才會產生 `.afterthread-state.json`，而且**不會**順手去刪 `tool.json`
  裡那個已經沒人讀的舊欄位：為了整潔去改寫 manifest，正好會移動這次改動要固定住的
  那個身分。**唯一的例外**：如果你的套件裡**本來就有**一個叫這個名字的檔案（而且不是
  後端寫的），看下面那一項——它會被忽略、被保留，而那一包的開關會按不動。
- **新裝的工具一律是開著的**，而且那是後端寫的、不是模型決定的：安裝流程在把套件搬
  進工具目錄之前，就先寫好一份 `{"afterthread": "tool-state", "enabled": true}`。
  否則模型只要在 `tool.json` 裡寫一句 `"enabled": false`，工具就會裝好就是關的、從
  沒被端給模型過，而你從沒按過任何開關。開關是你的，builder 沒有投票權。
- **那個名字上是我自己的檔案**（沒有所有權標記）——**後端當作沒看到，而且不動它**。
  `.afterthread-state.json` 在 web-v5 P1 之前不是保留名字，所以既有的工具可能剛好也用了
  這個名字放自己的東西。判準只有一個：讀得出來、但沒有 `"afterthread": "tool-state"`
  的內容（包含根本不是 JSON 的內容），就**不是後端的**。這時：
  - 那一包**照常有效、照常會被端給模型**（不會因此變成「無效」）；
  - 開關讀的是 `tool.json` 的舊欄位，跟「沒有狀態檔」完全一樣；
  - **`PATCH /api/tools/{name}` 會回 404 而不是把你的檔案蓋掉**——UI 上按那顆開關會
    失敗，那就是你會撞到的訊號；
  - 一次修訂會把你的檔案**逐位元組**帶過換裝，不會刪掉它；
  - `GET /api/tools` 那一列的 `error` 欄會寫明這件事。**誠實的限制**：畫面上只有
    「無效」的列才顯示錯誤文字，所以這句話在 UI 上看不到，要打 API 才看得到。
  **要修的話有兩條路，都得你自己動手**（那是你的檔案）：把它改成別的名字，或在裡面
  加上 `"afterthread": "tool-state"`——加了就等於把這個檔案交給後端管，後端之後會
  覆寫它。**特別提醒**：如果你自己手寫一份 `{"enabled": false}` 但**忘了加標記**，
  它會被安靜地忽略，工具照 `tool.json` 的值運作。
- **`.afterthread-state.json` 壞掉不會被當成「沒設定」**。這裡說的是**後端自己那份**壞掉：
  完全讀不出來（超過上限、被換成 symlink／FIFO／目錄），或帶著標記但 `enabled`
  不是布林。這代表**你的意圖不明**，這時
  後端把那個工具列成無效、並在列表的錯誤欄顯示 `.afterthread-state.json exists but is not
  readable`，同時當成停用。刻意不退回「預設開啟」：那等於把一個有人特意關掉的工具
  重新交到模型手上。（讀得出來但**沒有標記**的內容不算這一類，見上一項。）修法
  **看它壞成什麼樣子**，兩類不一樣：
  - **還是一般檔案，只是內容壞了**（帶標記卻沒有可用的 `enabled`、超過上限）
    ——一次 `PATCH /api/tools/{name}` 就修好：它只讀到「這不是別人的檔案」為止，
    然後直接覆蓋成乾淨的一份。
    **但畫面上那顆開關按不下去**：這個套件此刻正是「無效」的，而 UI 對每一個無效列
    都停用開關。所以實務上兩條路——**自己把 `.afterthread-state.json` 刪掉**（回到上面那條舊安裝
    的 fallback 規則，之後開關就能按了），或直接對該工具送一次 `PATCH`。
  - **已經不是一般檔案了**（被換成 symlink／FIFO／目錄）——**連 `PATCH` 也修不好**。
    發佈器在寫入前會拒絕任何既存的非一般檔（那是刻意的寫入邊界：絕不透過 symlink
    或 FIFO 寫出去），所以 PATCH 只會失敗回 404，那個東西原封不動。這時只能自己把
    它移除，移除後就回到上面那條舊安裝的 fallback 規則。
- **手改一樣被接住**。這個檔案跟套件裡其他東西一樣是你的，可以直接編輯；後端下一次
  掃描就會照著它走。後端讀它的時候只保證一件事、也只需要這一件：**讀到的一定是某一
  個完整版本，絕不是寫到一半的半份**（後端自己的寫入是「先寫暫存檔、再原子換上去」）。
  它不保證那個值就是「此刻」磁碟上最新的——版本在打開檔案的那一瞬間就選定了，而在
  一次工具呼叫前面加鎖去追求那個「此刻」，代價是每一次工具呼叫都要跟每一次開關排隊。
- **整個套件目錄被換成 symlink ⇒ 那個工具一律不跑**，而且清單上會顯示成「無效 ＋
  關著」。後端在接上任何檔名之前就拒絕看這種路徑（否則 `<連結>/.afterthread-state.json` 會跟著
  連結離開工具目錄），而「拒絕去看」的答案跟壞掉的狀態檔一樣是**停用**。
- **修訂或重新產生跑到一半，照樣可以按開關**。開關不再碰 `tool.json`，所以它既不會
  讓整場修訂作廢、也不會讓一趟總結往返最後的寫入被拒。落在修訂裡的一次切換會被帶過
  換裝（換裝前後端會把正式套件當下的狀態重新寫進暫存區），落在換裝那一瞬間的則會等
  換裝做完、寫到剛發佈的那一包——兩邊都算數。UI 上那顆開關因此不再被修訂／重新產生
  鎖住（無效套件與同一列已經在飛的切換／刪除仍會鎖）。
...
- **根層 `.env` 不進工作區，也不由模型改寫**。複製套件到暫存目錄時，**套件根層的
  ...
  該工具的 `.env`，這是 v1 明說接受的限制——但手寫的值必須是遮蔽器**遮得掉**的，
  否則修訂在開工前就被拒絕，兩種情形分開講、補救也不同：
  - **長度至少要 6 字元**：遮蔽器對更短的值一律略過（不然「1234」這種字串會把散文
    洗成一片遮蔽標記），補救是加長，或把它移出 `.env`；
  - **「決定該值的那一行」的右邊，要剛好等於這套系統自己寫得出來的拼法**：合格的只有
    三種——裸值、`'值'`、`"值"`，而且每一種要在序列化器認定安全時才算數（裸值必須是
    它本來就會寫成裸值的那種值；`'值'` 要求值裡沒有單引號；`"值"` 要求值裡沒有 `"`
    也沒有 `\`，跳脫不可能生效）。`KEY="ab'cd\"ef"` 這種跳脫真的會生效的寫法，解析出來
    是 `ab'cd"ef`，那一行的右邊並不是它——遮蔽器只認得解析後的值，所以 builder 一句
    `cat .env`（正式套件在暫存區外，但 `run_shell` 構得到）就把遮不掉的原文帶回對話。
    **看的是那一行，不是整份檔案**，而且是**等於**、不是**包含**：只要是「包含」，
    旁邊的文字就能替憑證作保——別處的一行註解可以，**同一行的行尾註解**也可以
    （`KEY="ab\"cd" # ab"cd`：dotenv 把引號值與註解分開吃，「值出現在右邊」照樣成立）。
    python-dotenv 是**後出現者勝**，所以要看的是**最後**一行指派該 key 的賦值；同一個 key
    前面那些被蓋掉的行不算數（它們解析不出任何已登記的值）。**找不到可對應的賦值行也一樣
    拒絕**——跨行的引號值、續行、任何以「一行」讀不出來的寫法：讀得出來的才驗，讀不出來的
    就拒絕。**代價要講清楚**：`KEY=值 # 註解`、`KEY=兩個 詞`、`KEY=a#b` 這些**合法但我們
    寫不出來**的形狀現在也被拒——它們本身不是外洩，但沒有任何比「重新實作 dotenv
    tokenizer」更短的規則能把它們跟上面那行分開，所以閘門只認自己生得出來的拼法。補救
    是把那一行寫成最單純的形式（原值直接寫、或整段加引號），並移除行尾註解與多餘跳脫。
    安裝路徑本來就拒絕**寫出**這種拼法（`_dotenv_serialize_value` 會直接放棄），這是同一條
    規則套在繼承端；反過來說，**凡是這套系統自己寫出來的 `.env` 一定過得了這一關**
    （它會把先前同名的行全部丟掉、自己的那行附在最後，而合格集合就是問同一個序列化器
    要來的）。

  理由都一樣：修訂會把 `.env` 的值餵進 `run_shell`，遮不掉就會隨輸出進到下一輪提示、
  AI 日誌，甚至總結。同理，`.env` 存在但**讀不到值**（symlink、權限錯誤、超過大小
  上限）也是整場拒絕，而不是當成「這個工具沒有 `.env`」；大小上限**以位元組計**
  （64 KiB），入口與換裝前的複製用的是同一把尺，所以換裝會拒的 `.env` 不會先燒掉
  一整場 session。

  **這整套政策在換裝前會對「即將出貨的那些位元組」再跑一次**。`.env` 可以在 session
  中途被建立或換掉——包含「開工時這個套件根本沒有 `.env`」的情況——而開工那次檢查只
  回答了**當時那個檔案**。所以複製前會重讀來源、對那些位元組重驗一次（拼法、長度下限、
  位元組上限），不合格就以同一組訊息**拒絕換裝**：閘門管的是真的會發佈出去的東西。
  重讀到的值也會**在複製之前**登記成「進行中的秘密」，讓換裝後的總結與 sidecar 路徑
  有遮蔽可用。

  **⚠️ 修訂進行中請不要動該套件的 `.env`。** 上面封住的是「不會把閘門會拒的東西發佈
  出去」；封不住的是**session 進行中的那次讀取**——你在修訂跑到一半時新建的憑證檔，
  builder 當下就 `cat` 得到，而那個值還沒有任何人登記過，AI 日誌又是**寫入當下**遮蔽、
  事後無法回頭補遮。要真正擋住只能把 `run_shell` 關進沙箱，而 v1 明文不做（見下一節的
  安全立場）。所以：要換憑證，等修訂結束再改。（完整裁決記在 repo 根目錄
  `裁決紀錄.md` #6。）
- **巢狀的 `.env` 是普通套件內容，會照常複製**。工具自己讀的 `config/.env` 之類的檔案
  ...
  比「靜默把那個檔案刪掉、然後驗證通過」好得多。**根層的大小寫變體（`.ENV`）同理**，
  而判準是**那個目錄自己的檔名列表**：列表裡**同時**有 `.env` 與 `.ENV`，就代表這是
  兩個不同的目錄項（在 case-sensitive 檔案系統上，即使兩者是 hard link 也一樣），
  `.ENV` 就是普通套件內容，照常複製、照常出貨；列表裡**只有變體那一個**（case-insensitive
  檔案系統上兩個名字本來就是同一項），而且 `<套件>/.env` 開出來確實就是它時，才排除。
  用 inode 當唯一判準會漏掉 hard link 那種「兩個目錄項、一個 inode」的情形，把一個
  普通檔案排除掉——而後端只會還原正好叫 `.env` 的那一個，所以誤排除等於把它靜默刪掉。
  同一項的確認刻意用**不跟隨**的 lstat：一條指向 `.env` 的 `.ENV` symlink 在跟隨式的
  比對下也會被當成「同一個檔案」，排除它就是同一個刪檔 bug 換個樣子。
- **啟用狀態同樣不進工作區，但會被帶過去**。`.afterthread-state.json`（見下一小節）跟 sidecar
  一樣是後端自己的檔案，複製套件時排除，builder 寫了一個也會在換裝前被剷掉——它是
  執行前決定「這個工具能不能跑」的那個檔案，一次修訂 session 不該有機會改它。但換裝
  是**整包換掉**，所以排除之後還得補回來：驗證通過、換裝之前，後端會讀正式套件現在
  的啟用狀態，用跟開關同一個原子寫法寫進暫存區。少了這一步，每一次修訂都會把你刻意
  關掉的工具靜默打開。寫不進去就**拒絕換裝**（訊息：無法保留工具的啟用狀態）——寧可
  放棄一次修訂，也不要發佈一個會自己開起來的工具。**兩個範圍要說清楚**：（一）排除
  的只有**套件根目錄**那一個，`data/.afterthread-state.json` 之類的巢狀同名檔案是你的工具
  自己的內容，照常複製、照常出貨（跟巢狀 `.env` 同一條規則）；（二）如果根目錄那個
  名字上放的是**你自己的檔案**（沒有所有權標記），這一步改成把它**逐位元組複製**回去，
  不會用 `{"enabled": ...}` 蓋掉它。
...
- **修訂完會重新產生總結**：sidecar 沒被複製進工作區，所以換裝後那一刻它是沒有
  ...
  AI 迭代」的意思。**修訂工作自己開工前還會再嚴格讀一次 sidecar**：讀不出可信狀態
  （檔案壞掉、被手改成不是 JSON、讀取被拒）就直接拒絕，訊息與換裝前那道閘同一組
  （兩種補救分開講：修好／移除損壞的 sidecar vs. 解除定版）。這道嚴格的閘在**工作**
  這一側而不是路由那一側，是因為換裝前那一關本來就會拒同一種檔案——不先擋掉，就是
  白燒一整場多輪 builder session（而且整段期間佔住「一次只跑一個」的名額），重試一次
  再燒一次。路由的那道入口檢查刻意維持寬鬆版本：路由回答的是「這個資源目前已知的
  狀態」，猜錯的代價就是這個拒絕改以工作結果呈現，而不是當下的 409。
- **一次只跑一個**：修訂和安裝共用同一張工作表與同一個輪詢端點
  （`GET /api/tools/jobs/{job_id}`），任何一個工作在跑的時候都擋下新的工作，也
  擋下「重新產生總結」——搬整個目錄的那個窗口裡，同時讀寫同一個套件是在跟自己
  賽跑。**反方向同樣成立**：同步的「重新產生總結」會**佔住同一個名額**，從它開始
  到 LLM 回來為止，安裝與修訂的送出都會被 409 擋下。少了這一半，窗內被放行的修訂
  會換掉整包、寫下**新的** sidecar，然後被那次較舊的總結（內容是**已經被換掉**的
  套件）蓋回去。
...
- 每一輪 attempt 另外記下**當輪廣告給模型的工具名單**（`tools_advertised`），
  日誌頁會在該輪的統計行下面用標籤列出來。這些名稱和訊息內容走**同一個**存檔
  關卡（遮蔽→編碼安全→截斷），因為一個工具名稱本來就可能剛好等於某個已登記的
  秘密值（手編的 `.env` 寫了 `TOKEN=kbsearch`，而 `kbsearch` 正好是已安裝的工具）
  ——紀錄裡不能有任何一個欄位繞過那道遮蔽。工具規格是跟著 API 參數送的、不在
  訊息內容裡，所以這是唯一能看出「模型當下看得到哪些工具」的地方。值為 `null`
  代表那一輪完全沒送工具參數——例如沒有啟用任何工具、輪次或篇幅預算用完後的
  收尾那一輪、以及格式修正重試的那一輪。只記名稱不記完整規格：單一工具的參數
  schema 可以很大，逐輪重複存會把紀錄撐爆。要留意這代表**規格沒有歷史**：日誌
  說的是「那一輪端出了哪些工具名稱」，而工具包裡的 `tool.json` 顯示的是**目前
  這一版**的規格——AI 修訂會就地換掉同一個檔名，所以對任何早於最後一次修訂的
  紀錄來說，那份 manifest 已經不是當時送出去的那份，而舊的規格本系統沒有留。
